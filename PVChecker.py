...
def run_once():
    ...
    home_assistant_entity_id = os.getenv(
    ...
    try:
        ...
        if disable_homeassistant:
            print(
                f"[DRY RUN] Would send to Home Assistant entity '{home_assistant_entity_id}':", ha_payload)
        else:
            headers = {
        ...
        print("Serial connection or upload failed on %s: %s" %
              (serial_target, inst))
        return 1
    finally:
        port.close()
